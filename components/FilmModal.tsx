...
const FilmModal: React.FC<FilmModalProps> = ({ film, log, onUpdateLog, onClose, onNavigateToList, sherpaNote, isEditing, onSaveNote, isUGC, listTitle }) => {
  ...
  const [realDetails, setRealDetails] = useState<{director: string, cast: string[], runtime: number, screenplay: string[], music: string[]} | null>(null);
  ...
  useEffect(() => {
    ...
    if (film) {
      ...
      if (film.isCustomEntry) {
         setAnalysis({ summary: "Custom entry.", significance: "User curated.", funFact: "-", director: film.director, cast: film.cast || [], year: film.year });
      } else {
        setLoading(true);
        setAnalysis(null);
  ...
  // Veri önceliği: Önce TMDB, yoksa Gemini, yoksa Film objesindeki varsayılan
  const displayDirector = realDetails?.director || analysis?.director || film.director;
      ...
      <div className="relative w-full max-w-5xl bg-[#F5C71A] border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-6 md:p-8 max-h-[90vh] overflow-y-auto text-black">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="md:w-1/3 flex-shrink-0 flex flex-col items-center md:items-start space-y-4">
            ...
            <div className="w-full border-4 border-black bg-black text-[#F5C71A] p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
               ...
               <div className="w-full">
                 <div className="flex justify-between items-end"><span className="text-xs font-bold uppercase">Rating</span><span className="text-xl font-black">{rating > 0 ? rating : '-'}</span></div>
                 <RatingBar />
               </div>
            </div>
            ...
            <div className="w-full flex justify-between gap-2 text-xs font-mono font-bold">
                ...
                <div className="flex-1 bg-black text-[#F5C71A] border border-[#F5C71A] p-1 flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm">{film.imdbScore ? film.imdbScore : '-'}</span>
                </div>
                <div className="flex-1 bg-black text-[#F5C71A] border border-[#F5C71A] p-1 flex items-center justify-center gap-2">
            ...
            <div className="w-full text-black border-2 border-black bg-white/20 p-3 text-sm space-y-2">
               ...
               <div className="grid grid-cols-[80px_1fr] gap-2 border-b border-black/20 pb-2"><span className="font-bold uppercase opacity-70">Music</span><span className="font-medium italic">{displayMusic ? displayMusic.join(', ') : '-'}</span></div>
               <div className="grid grid-cols-[80px_1fr] gap-2 border-b border-black/20 pb-2"><span className="font-bold uppercase opacity-70">Cast</span><span className="italic">{displayCast ? displayCast.slice(0, 4).join(', ') : 'Unknown'}</span></div>
          ...
          <div className="md:w-2/3 flex-grow flex flex-col gap-6">
            ...
            {loading ? <div className="animate-pulse font-bold">Consulting Archives...</div> : analysis && (
              <div className="space-y-6">
                <div className="bg-white/20 p-4 border-l-4 border-black"><h3 className="font-black text-lg mb-2 uppercase">The Setup</h3><p className="text-lg font-medium">{analysis.summary}</p></div>
                <div><h3 className="font-black text-lg mb-2 uppercase">Why This Film?</h3><p className="text-lg">{analysis.significance}</p></div>
                <div className="bg-black/5 p-4 border border-black/20"><h3 className="font-black text-xs mb-1 uppercase">★ Classified Info</h3><p className="text-sm font-mono opacity-80">{analysis.funFact}</p></div>
              </div>
            )}
          </div>
        </div>
