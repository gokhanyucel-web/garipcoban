    ...
    <div className="min-h-screen w-full bg-[#F5C71A] flex items-center justify-center p-6 fixed inset-0 z-50">
      <div className="w-full max-w-md bg-black text-[#F5C71A] p-8 border-4 border-black shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] relative">
        ...
        <div className="flex justify-center mb-6 border-b border-[#F5C71A]/30 pb-4">
           <button onClick={() => setMode('signin')} className={`px-4 py-2 text-xs font-bold uppercase tracking-widest ${mode === 'signin' ? 'text-[#F5C71A] border-b-2 border-[#F5C71A]' : 'text-gray-500'}`}>Enter</button>
           <button onClick={() => setMode('signup')} className={`px-4 py-2 text-xs font-bold uppercase tracking-widest ${mode === 'signup' ? 'text-[#F5C71A] border-b-2 border-[#F5C71A]' : 'text-gray-500'}`}>Join</button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'signup' && <div><label className="block text-xs font-bold uppercase mb-1">Codename</label><input className="w-full p-3 bg-[#222] border-2 border-[#F5C71A] text-[#F5C71A]" type="text" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} required /></div>}
          <div><label className="block text-xs font-bold uppercase mb-1">Email</label><input className="w-full p-3 bg-[#222] border-2 border-[#F5C71A] text-[#F5C71A]" type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required /></div>
          <div><label className="block text-xs font-bold uppercase mb-1">Passkey</label><input className="w-full p-3 bg-[#222] border-2 border-[#F5C71A] text-[#F5C71A]" type="password" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} required /></div>
          {mode === 'signup' && <div><label className="block text-xs font-bold uppercase mb-1">Confirm</label><input className="w-full p-3 bg-[#222] border-2 border-[#F5C71A] text-[#F5C71A]" type="password" value={formData.confirmPassword} onChange={(e) => setFormData({...formData, confirmPassword: e.target.value})} required /></div>}
  ...
  <div className="min-h-screen w-full bg-[#F5C71A] text-black font-sans selection:bg-black selection:text-[#F5C71A] flex flex-col transition-colors duration-300">
    <header className="pt-12 pb-8 text-center px-4 relative">
         <div className="absolute top-8 right-8 flex gap-4">
             {isAdmin && <span className="bg-red-600 text-white px-2 py-1 text-xs font-black uppercase border border-black animate-pulse">ADMIN</span>}
             {!session ? (
...
function App() {
  ...
  const fetchUserData = async (userId: string) => {
    try {
        ...
        if (lists) {
            // FIX: Ensure list is marked as custom
            const mappedLists: CuratedList[] = lists.map((item: any) => ({ 
                ...item.content, 
                id: item.id, 
                status: item.status, 
                author: profile.name, 
                isCustom: true // This is crucial
            }));
  ...
  const handleForkList = () => {
    ...
    if (session) {
        ...
        // FIX: Insert into Custom Lists AND Vault
        supabase.from('custom_lists').upsert(payload).then(({ error }) => {
            if(error) console.error("Auto-save error:", error);
            else {
                 // AUTO-ADD TO VAULT
                 supabase.from('vault').insert({ user_id: session.user.id, list_id: newId });
  ...
  const handleSaveList = async () => {
    ...
    if (listToSave.id.startsWith('custom_')) {
        setCustomLists(prev => {
            ...
            return exists ? prev.map(l => l.id === listToSave.id ? listToSave : l) : [...prev, listToSave];
        });
  ...
  const handleCreateList = () => {
      if(!session) { setView('auth'); return; }
      const newId = `custom_${Date.now()}`;
      const newList: CuratedList = { id: newId, title: "UNTITLED", subtitle: "Custom", tiers: [{level:1, name:"TIER 1", films:[]}], isCustom:true, author:profile.name, status:'draft' };
      setCustomLists(prev => [...prev, newList]); setSelectedList(newList); setEditingList(newList); setIsEditorMode(true); setIsAICreatorOpen(false);
  };
  ...
  const handleGenerateAndCreate = async () => {
      ...
      setCustomLists(prev => [...prev, newList]); setSelectedList(newList); setEditingList(newList); setIsEditorMode(true); setIsAICreatorOpen(false); setIsGeneratingAI(false);
  };
  ...
  const getVaultLists = () => {
    ...
    // FIX: Include if in Vault OR if it's my custom list
